...
def count(node, value):
    """
        Shell function for countRec
        node = linked list to count
        value = value to search for
    """
    #return countRec(node.head, value)

def countRec(node, value):
    """
        Counts the instances of value in node recursively
        node = linked list
        value = value to search for
        returns in
    """
    if node == None:
        return 0
    if node.data == value:
        return 1 + count(node.next, value)
    else:
        return count(node.next, value)

def remove(node, value):
    """
        Shell function for removeRec
        node = linked list
        value = value to remove
    """
    #removeRec(node.head, value)

def removeRec(node, value):
    """
        Removes the first instance of value from node recursively
        node = linked list
        value = value to remove
    """
    if node == None:
...
def myListToPyList(list):
    """
        Shell for myListToPyList
        list = linked list
    """
    pyList = []
    myListToPyListRec(list.head, pyList)

def myListToPyListRec(list, pyList):
    """
        Converts pyList to pyList recursively
        list = linked list
    """
    if list != None:
        pyList.append(list.data)
        list = list.next
    else:
        return pyList
    return myListTopyList(list, pyList)

def pyListToMyList(list):
    """
        Shell for pyListToMyListRec
        list = linked list
    """
    list2 = list[:]
    myList = createEmptyList()
    pyListToMyListRec(list2, myList)

def pyListToMyListRec(list, myList):
    """
        Converts pyList to myList recursively
        list = linked list
    """
    if list != []:
        append(myList, list[0])
    else:
        return myList
    return myList(list[1:], myList)
