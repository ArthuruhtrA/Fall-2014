...
def count(list, value):
    """
        Counts the instances of value in list
        list = linked list
...
def remove(list, value):
    """
        Removes the first instance of value from list
        list = linked list
        value = value to remove
    """
    if list.size == 1 and list.head.data == value:
            list.head = None
            list.size -= 1
            return True
    node = list.head
    while node != None:
        if node.next != None and node.next.data == value:
